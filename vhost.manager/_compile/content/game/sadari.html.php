<?php /* Template_ 2.2.3 2016/05/04 00:54:39 C:\inetpub\web\5. Armand De\www\vhost.manager\_template\content\game\sadari.html */
	...
	<div id="search">
		<form name=frmSrh method=post action="/game/sadari">
			...
			<div class="betList_option">
				...
				<span>출력</span>
				<input name="perpage" type="text" id="perpage"  class="sortInput" onkeyup="if(event.keyCode !=37 && event.keyCode != 39) value=value.replace(/\D/g,'');" maxlength="4" size="5" value="<?php echo $TPL_VAR["perpage"]?>" onmouseover="this.focus()">
				...
				<select name="sadari_type" onchange="submit()">
					<option value="1"  <?php if($TPL_VAR["sadari_type"]==1){?>  selected <?php }?>>홀/짝</option>
					<option value="2"  <?php if($TPL_VAR["sadari_type"]==2){?>  selected <?php }?>>3줄/4줄</option>
					<option value="3"  <?php if($TPL_VAR["sadari_type"]==3){?>  selected <?php }?>>좌출/우출</option>
					<option value="4"  <?php if($TPL_VAR["sadari_type"]==4){?>  selected <?php }?>>좌3/좌4</option>
					<option value="5"  <?php if($TPL_VAR["sadari_type"]==5){?>  selected <?php }?>>우3/우4</option>
				</select>
...
<?php }?>
						...
						<td><?php echo number_format($TPL_V1["home_rate"],2)?></td>
						<td><?php if(($TPL_V1["draw_rate"]=='1.00'&&$TPL_V1["type"]==1)||$TPL_V1["draw_rate"]=='0'||$TPL_V1["draw_rate"]==''){?>VS<?php }else{?><?php echo number_format($TPL_V1["draw_rate"],2)?><?php }?></td>
						<td><?php echo number_format($TPL_V1["away_rate"],2)?></td>
						<td class="awayName">
							<a href=javascript:team_betting2("/game/popup_gamedetail?child_sn=<?php echo $TPL_V1["child_sn"]?>"); style='cursor:hand' onmousemove="showpup('<?php echo $TPL_V1["home_team"]?>&nbsp;&nbsp;VS&nbsp;&nbsp;<?php echo $TPL_V1["away_team"]?>')" onmouseout='hidepup()'>
