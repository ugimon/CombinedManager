<?php /* Template_ 2.2.3 2016/06/20 01:59:06 C:\inetpub\combined_manager\vhost.manager\_template\content\config\global_config.html */
...
<script>
	function confirmOk()
	{
		...
		else if(fm.ladder_bettingendtime.value=="")
		{
			alert("사다리 배팅마감시간을 입력해 주세요.");
			fm.ladder_bettingendtime.focus();
		}
		else if(fm.powerball_bettingendtime.value=="")
		{
			alert("파워볼 배팅마감시간을 입력해 주세요.");
			fm.powerball_bettingendtime.focus();
		}
		else if(fm.snail_bettingendtime.value=="")
		{
			alert("달팽이 배팅마감시간을 입력해 주세요.");
			fm.snail_bettingendtime.focus();
		}
...
<?php if($TPL_logolist_1){foreach($TPL_VAR["logolist"] as $TPL_V1){?>
						<input type="radio" name="logo" value="<?php echo $TPL_V1["name"]?>" <?php if($TPL_VAR["logo"]==$TPL_V1["name"]){?> checked <?php }?>><?php echo $TPL_V1["nick"]?>
...
<?php }}?>
	...
	<form  method="post"  name="frm">
	    ...
	    <tr>
				<th width="20%">첫충/매충(%)</th>
				<td>
				레벨1 - 첫충 <input name="first_charge1" type="text"  class="w60" id="first_charge1" value="<?php echo $TPL_VAR["rat1"]["firstRate"]?>" maxlength="30" />%  매충 <input name="every_charge1" type="text"  class="w60" id="every_charge1" value="<?php echo $TPL_VAR["rat1"]["everyRate"]?>" maxlength="30" />%<br/>

				레벨2 - 첫충 <input name="first_charge2" type="text"  class="w60" id="first_charge2" value="<?php echo $TPL_VAR["rat2"]["firstRate"]?>" maxlength="30" />%  매충 <input name="every_charge2" type="text"  class="w60" id="every_charge2" value="<?php echo $TPL_VAR["rat2"]["everyRate"]?>" maxlength="30" />%<br/>

				레벨3 - 첫충 <input name="first_charge3" type="text"  class="w60" id="first_charge3" value="<?php echo $TPL_VAR["rat3"]["firstRate"]?>" maxlength="30" />%  매충 <input name="every_charge3" type="text"  class="w60" id="every_charge3" value="<?php echo $TPL_VAR["rat3"]["everyRate"]?>" maxlength="30" />%<br/>

				레벨4 - 첫충 <input name="first_charge4" type="text"  class="w60" id="first_charge4" value="<?php echo $TPL_VAR["rat4"]["firstRate"]?>" maxlength="30" />%  매충 <input name="every_charge4" type="text"  class="w60" id="every_charge4" value="<?php echo $TPL_VAR["rat4"]["everyRate"]?>" maxlength="30" />%<br/>

				레벨5 - 첫충 <input name="first_charge5" type="text"  class="w60" id="first_charge5" value="<?php echo $TPL_VAR["rat5"]["firstRate"]?>" maxlength="30" />%  매충 <input name="every_charge5" type="text"  class="w60" id="every_charge5" value="<?php echo $TPL_VAR["rat5"]["everyRate"]?>" maxlength="30" />%
				</td>
	    </tr>
	    <tr>
			<th>은행계좌</th>
	    ...
	    <tr>
			<th>배팅마감</th>
			<td>
			경기시간 <input type="text" class="w60" name="bettingendtime" value="<?php echo $TPL_VAR["list"]["bettingendtime"]?>" onKeyUp="value=value.replace(/[^\d]/g,'')" onbeforepaste="clipboardData.setData('text',clipboardData.getData('text').replace(/[^\d]/g,''))" />초전
			</td>
		</tr>
		<tr>
			<th>웹게임 배팅마감</th>
			<td>
			사다리 경기시간 <input type="text" class="w60" name="ladder_bettingendtime" value="<?php echo $TPL_VAR["list"]["ladder_bettingendtime"]?>" onKeyUp="value=value.replace(/[^\d]/g,'')" onbeforepaste="clipboardData.setData('text',clipboardData.getData('text').replace(/[^\d]/g,''))" />초전&nbsp;&nbsp;
			파워볼 경기시간 <input type="text" class="w60" name="powerball_bettingendtime" value="<?php echo $TPL_VAR["list"]["powerball_bettingendtime"]?>" onKeyUp="value=value.replace(/[^\d]/g,'')" onbeforepaste="clipboardData.setData('text',clipboardData.getData('text').replace(/[^\d]/g,''))" />초전&nbsp;&nbsp;
			달팽이 경기시간 <input type="text" class="w60" name="snail_bettingendtime" value="<?php echo $TPL_VAR["list"]["snail_bettingendtime"]?>" onKeyUp="value=value.replace(/[^\d]/g,'')" onbeforepaste="clipboardData.setData('text',clipboardData.getData('text').replace(/[^\d]/g,''))" />초전
			</td>
	    </tr>
	    <tr>
			<th>배팅취소허용시간</th>
