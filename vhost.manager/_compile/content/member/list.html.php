<?php /* Template_ 2.2.3 2016/06/22 04:18:16 C:\inetpub\combined_manager\vhost.manager\_template\content\member\list.html */
...
<?php if($TPL_logolist_1){foreach($TPL_VAR["logolist"] as $TPL_V1){?>
					<option value=<?php echo $TPL_V1["name"]?> <?php if($TPL_VAR["filter_logo"]==$TPL_V1["name"]){?> selected <?php }?>><?php echo $TPL_V1["nick"]?></option>
<?php }}?>
				</select>
			...
			<select name="sort_field">
				...
				<option value="regdate" <?php if($TPL_VAR["sort_field"]=="regdate"){?> selected <?php }?>>가입일시</option>
				<option value="g_money" <?php if($TPL_VAR["sort_field"]=="g_money"){?> selected <?php }?>>보유머니</option>
				<option value="virtual_money" <?php if($TPL_VAR["sort_field"]=="virtual_money"){?> selected <?php }?>>가상머니</option>
				<option value="benefit" <?php if($TPL_VAR["sort_field"]=="benefit"){?> selected <?php }?>>회원수익</option>
				<option value="visit_count" <?php if($TPL_VAR["sort_field"]=="visit_count"){?> selected <?php }?>>접속횟수</option>
				<option value="recent_visit" <?php if($TPL_VAR["sort_field"]=="recent_visit"){?> selected <?php }?>>최근 접속일</option>
			</select>
...
$TPL_permit_domain_list_2=empty($TPL_V1["permit_domain_list"])||!is_array($TPL_V1["permit_domain_list"])?0:count($TPL_V1["permit_domain_list"]);?>
				<tr bgcolor=<?php echo $TPL_V1["bgColor"]?>>
					...
					<td rowspan="2"><font color='green'><?php echo number_format($TPL_V1["virtual_money"],0)?></font></td>
					<td rowspan="2">
...
<?php }}?>
				...
				<tr bgcolor=<?php echo $TPL_V1["bgColor"]?>>
					<td>
						<font color="<?php echo $TPL_V1["logo_color"]?>"><?php echo $TPL_V1["logo_nick"]?></font>
					</td>
