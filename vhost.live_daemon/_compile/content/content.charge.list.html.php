<?php /* Template_ 2.2.3 2012/10/10 00:24:36 C:\APM_Setup\htdocs\www\vhost.manager\_template\content\content.charge.list.html */
...
<?php if($TPL_list_1){foreach($TPL_VAR["list"] as $TPL_V1){?>
		<tr>
			<td><input name="y_id[]" type="checkbox" id="y_id" value="<?php echo $TPL_V1["money_idx"]?>"  onclick="javascript:chkRow(this);"/></td>
			<td><?php echo $TPL_V1["regdate"]?></td>        
			<td><?php echo $TPL_V1["operdate"]?></td>
			<td><a href="javascript:open_window('/member/popup_detail?idx=<?php echo $TPL_V1["idx"]?>',750,580)"><?php echo $TPL_V1["mem_id"]?></td>
			...
			<td><?php if($TPL_V1["todaycount"]==''){?>0<?php }else{?><?php echo $TPL_V1["todaycount"]?><?php }?>/<?php echo $TPL_V1["rechargenum"]?></td>
			<td><?php if($TPL_V1["result"]==0){?><font color='red'>신청</font><?php }elseif($TPL_V1["result"]==1){?><font color='blue'>완료</font><?php }else{?><font color='yellow'>삭제</font><?php }?></td>
			<td><a href="javascript:open_window('/charge/agree?idx=<?php echo $TPL_V1["idx"]?>&rmoney=<?php echo $TPL_V1["amount"]?>&money_idx=<?php echo $TPL_V1["money_idx"]?>',400,250)"><img src="/img/btn_s_confirm2.gif" title="승인"></a></td>
		</tr>
<?php }}?>
	</tbody>
	</table>
